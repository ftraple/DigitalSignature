...
namespace {
    struct BIOFreeAll { void operator()(BIO* p) {
        BIO_free_all(p);
    }};
}
...
const std::string Base64Encode(const std::vector<unsigned char>& binaryMessage) { 
    ...
    BIO* sink = BIO_new(BIO_s_mem());  // TODO Check for memory leak
...
const std::vector<unsigned char> Base64Decode(const std::string& base64Message) {
    std::unique_ptr<BIO, BIOFreeAll> b64(BIO_new(BIO_f_base64()));
    BIO_set_flags(b64.get(), BIO_FLAGS_BASE64_NO_NL);
    BIO* source = BIO_new_mem_buf(base64Message.c_str(), -1); // TODO Check for memory leak
